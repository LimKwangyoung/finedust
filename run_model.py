...
def run_model(csv_path: str, draw_option = False):
    ...
    def train_test_split_window(train_df, test_df, win_size):
        ...
        #     X_train.append(np.array(train_df[['SO2', 'CO', 'O3', 'NO2']].iloc[i:i + win_size,]))
        #     y_train.append(np.array(train_df['PM10'].iloc[i:i + win_size, ]))
        # for i in range(test_df.shape[0] - win_size):
        #     X_test.append(np.array(train_df[['SO2', 'CO', 'O3', 'NO2']].iloc[i:i + win_size, ]))
        #     y_test.append(np.array(train_df['PM10'].iloc[i:i + win_size, ]))
        #
        # return np.array(X_train), np.array(y_train), np.array(X_test), np.array(y_test)

    X_train_df, y_train_df, X_test_df, y_test_df = train_test_split_window(train_set_scaled, test_set_scaled, window_size)
    ...
    # print(X_train.shape, y_train.shape, X_test.shape, y_test.shape) # (2294, 30, 1) (2294, 1) (537, 30, 1) (537, 1)

    lstm = LSTM_model(X_train, y_train, window_size)
